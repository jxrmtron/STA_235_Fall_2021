...
getSymbols(Symbols = c("AWR"), from="2016-01-01", to="2021-01-01",
           auto.assign=TRUE, warnings=FALSE, periodicity = 'monthly')

getSymbols(Symbols = c("OSTK"), from="2016-01-01", to="2021-01-01",
           auto.assign=TRUE, warnings=FALSE, periodicity = 'monthly')

getSymbols(Symbols = c("^GSPC"), from="2016-01-01", to="2021-01-01",
           auto.assign=TRUE, warnings=FALSE, periodicity = 'monthly')

Beta_DF<- as.data.frame(`AWR`)

Beta_DF <- Beta_DF %>%
  mutate( AWRreturns = monthlyReturn(AWR) )
...
Beta_DF <- Beta_DF %>%
  mutate( OSTKreturns = monthlyReturn(OSTK) )

nrow_Beta <- NROW(Beta_DF)


Beta_DF <- Beta_DF %>%
  mutate( Time = 1:nrow_Beta)

ggplot(Beta_DF, aes(x=Time)) + 
  geom_line(aes(y = SPreturns), color = "darkred") + 
  geom_line(aes(y = OSTKreturns), color="steelblue") +
  geom_line(aes(y = AWRreturns), color="cyan") 
  

SP_Model <- lm(SPreturns~SPreturns, data = Beta_DF)

AWR_Model <- lm(AWRreturns~SPreturns, data = Beta_DF)

OSTK_Model <- lm(OSTKreturns~SPreturns, data = Beta_DF)
